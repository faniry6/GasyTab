...
export default class CustomHtmlDivFormatter {
  private isStartOfTabs(line: Line) {
    return line.items.some(
      i => i instanceof ChordSheetJS.Tag && i.name == 'sot',
    );
  }
  private isEndOfTabs(line: Line) {
    return line.items.some(
      i => i instanceof ChordSheetJS.Tag && i.name == 'eot',
    );
  }
  ...
  format(song: Song, fontSize = 14) {
    ...
    song.lines.forEach((l, index) => {
      if (l.isEmpty()) html += '<div class="empty-line"></div>';
      if (waitEndOfTabs) {
      ...
      l.items.forEach(item => {
        if (item instanceof ChordSheetJS.ChordLyricsPair) {
          if (item.chords) {
            ...
            html += `<span class="chord${CHORD_SIZE_CLASS}">${
              item.chords
            }</span>${lyrics}`;
          } else {
            html += `${item.lyrics}`;
          ...
          if (item.name == 'comment' && item.value) {
            ...
            html += `<span class="${item.name}">Source website: ${
              item.value
            }</span>`;
